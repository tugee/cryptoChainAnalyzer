...
class NewEventsView:
    ...
    def _intitialize(self):
        ...
        button_start = ttk.Button(
          ...
          command=lambda: self._transactions_handler(str(self._wallet_id.get()))
        )
        ...
        columns = ["Timestamp","Transaction hash","To address","From address","Amount","Gas fee"]
    ...
    def _get_all_transactions_db(self, filter=None):
        ...
        for transaction in transactions_filtered:
            self._contract_list.insert('','end',values=(transaction.timestamp,transaction.transaction_hash,transaction.to_address,transaction.from_address,transaction.amount,transaction.gas_fee))
